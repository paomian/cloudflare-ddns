...
def updateLoadBalancer(ip):
    ...
    for option in config["load_balancer"]:
        ...
        if pools:
            ...
            response = cf_api(
                f'user/load_balancers/pools/{option["pool_id"]}',
                "PATCH",
                option,
                {},
                data,
...
def cf_api(endpoint, method, config, headers={}, data=False):
    ...
    try:
        ...
        if response.ok:
            ...
            print(
                "😡 Error sending '" + method + "' request to '" + response.url + "':"
            )
            print(response.text)
            return None
        ...
        if len(sys.argv) > 1:
            if sys.argv[1] == "--repeat":
                if ipv4_enabled and ipv6_enabled:
                    print(
                        "🕰️ Updating IPv4 (A) & IPv6 (AAAA) records every "
                        + str(ttl)
                        + " seconds"
                    )
                    ...
                    print(
                        "🕰️ Updating IPv6 (AAAA) records every " + str(ttl) + " seconds"
                    )
                next_time = time.time()
                killer = GracefulExit()
                prev_ips = None
